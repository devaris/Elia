function Match(exerciseRef, DOMposition){
   ...
   var __construct = function(that) {
	...
	var prepareMatchContent = function(exerciseRef){
		...
		{
			activitySelections[i]=undefined;
		...
		for (var h=0; h<Object.size(sets)-1; h++){
			...
			for (i=0;i<set1Lis.length;i++){	
				...
				if ((exerciseRef.settings.typeOfCheck.type) && (exerciseRef.settings.typeOfCheck.type == "individual")) {
					...
					theCheck.setAttribute('src', spa.CHECK_ICON);
			...
			case "selection":							
				...
				if (x.length>0){
					for (var i=0;i<x.length;i++){	
						myConn = x[i];	
						if ((myConn.sourceId == info.newSourceId) && (myConn.targetId == info.newTargetId) && (myConn.scope == info.newTargetEndpoint.scope)){
							myConn.removeClass("checkCorrect");
							myConn.removeClass("checkWrong");
							break;
						}							
			...
			for (var i=0;i<allConns.length;i++){
				...
				if (connExercise==exercisePrefix){
					if ((myScope.substr(scopePrefix.length)!='EX')&&(myScope.substr(scopePrefix.length)!='SAns')){
						...
						if ((exerciseRef.settings.typeOfCheck.type != "individual")||(exerciseRef.settings.typeOfCheck.type == "individual")&&((myItem.parentNode.id.substr(myItem.parentNode.id.lastIndexOf(delimItem)+1)==indexL))){
							myConn.removeClass("checkCorrect");
							myConn.removeClass("checkWrong");
							var foundIt = false;			
