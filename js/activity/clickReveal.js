function ClickReveal(exerciseRef, DOMposition, revealSub, audioFilename){
		
    //private properties
    var score = 0;
    var myHighlight;
    var mySignature = "clickReveal";
    var displayNS = this;
    var music; 
	var duration; 
	var pButton;
	...
	var makeTheReveals = function(theString){
		var searchPattern = new RegExp;
		searchPattern = /(\@\@)(\[)(.*?)(\])(.*?)(\@\@)/g;
		var replacePattern = '<div class="showMe" data-id="$3">$5</div>';
		return theString.replace(searchPattern, replacePattern);
	}
    ...
    var prepareDisplayContent = function(exerciseRef) {
		var theJSONElements = "";
		var htmlData = exerciseRef.activity.htmlData;
		
		theJSONElements += '<br>';
		var tempString = makeClickReveal(htmlData);
		theJSONElements += makeTheReveals(tempString);
		//theJSONElements += htmlData;
		theJSONElements += '<br><br>';
	...
	var assignCode = function() {	
		...
		/*
			...
			$(".answers").height(maxHeight);
*/
	
			//Function for click and show the text
